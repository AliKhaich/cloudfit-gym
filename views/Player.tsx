...
export const Player: React.FC<PlayerProps> = ({ workout, onClose }) => {
    ...
    peer.on('open', () => {
      ...
      remoteDisplayIds.forEach(id => {
        ...
        conn.on('open', () => {
          ...
          conn.send({
            type: 'HEARTBEAT',
            payload: { workout, currentModuleIndex, endTime: endTimeRef.current, isPaused, hostTime: Date.now() }
          });
        });
  ...
  useEffect(() => {
    if (currentModule) {
      ...
      if (ex) {
        setExercise(ex);
        const duration = currentModule.duration ?? ex.duration ?? 0;
  ...
  useEffect(() => {
    ...
    timerRef.current = window.setInterval(() => {
      ...
      if (remaining <= 0 && !isInitialLoading) {
  ...
  }, [isPaused, currentModuleIndex, isInitialLoading, workout.modules.length]);
