...
export const Prepare: React.FC<PrepareProps> = ({ workout, onStart, onEdit, onClose }) => {
  ...
  return (
    <div className="min-h-screen bg-[#1A1A1A] text-white animate-in slide-in-from-bottom duration-500 flex flex-col">
      <div className="p-6 flex items-center justify-between border-b border-white/5">
        <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
        <div className="text-center">
          <h2 className="text-xl font-black uppercase italic tracking-tighter truncate max-w-[180px]">{workout.name}</h2>
          <p className="text-[10px] font-black text-[#E1523D] uppercase tracking-widest">{calculateTotalTime()}</p>
        </div>
        <button onClick={() => onEdit(editedWorkout)} className="p-2 text-gray-400 hover:text-white transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="bg-white/5 rounded-[32px] p-6 border border-white/5 mb-8">
           <h3 className="text-[11px] font-black text-gray-500 uppercase tracking-widest mb-4">Preparation Checklist</h3>
           <div className="space-y-3">
              <div className="flex items-center gap-3">
                 <div className="w-5 h-5 rounded-full border-2 border-[#E1523D] flex items-center justify-center">
                    <div className="w-2 h-2 bg-[#E1523D] rounded-full"></div>
                 </div>
                 <span className="text-sm font-medium">Connect to paired displays</span>
              </div>
              <div className="flex items-center gap-3">
                 <div className="w-5 h-5 rounded-full border-2 border-[#E1523D] flex items-center justify-center">
                    <div className="w-2 h-2 bg-[#E1523D] rounded-full"></div>
                 </div>
                 <span className="text-sm font-medium">Adjust exercise durations below</span>
              </div>
           </div>
        </div>
        ...
        {hasModules ? editedWorkout.modules.map((m, idx) => {
               ...
               <div className="flex items-center justify-between border-t border-white/5 pt-3">
                  <div className="flex items-center gap-2 relative">
                    ...
                    <select 
                      ...
                      className="text-[10px] font-black uppercase tracking-widest bg-transparent outline-none pr-4"
                    >
                      <option value={LOCAL_DISPLAY_ID} className="bg-[#1A1A1A]">This Device</option>
                      {displays.map(d => (
                        <option key={d.id} value={d.id} className="bg-[#1A1A1A]">{d.name}</option>
                      ))}
                    </select>
                  </div>
                  <div className="text-[9px] font-black uppercase tracking-widest text-[#E1523D]">
                    Ready
                  </div>
               </div>
            </div>
          ...
          <div className="py-20 text-center flex flex-col items-center gap-4 bg-white/5 rounded-[40px] border-2 border-dashed border-white/5">
            <svg className="w-12 h-12 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">No exercises in this workout</p>
            <button onClick={() => onEdit(editedWorkout)} className="text-[#E1523D] text-[10px] font-black uppercase tracking-widest">Edit Modules</button>
          </div>
        )}
      </div>
      ...
      <div className="p-8 bg-gradient-to-t from-black to-transparent">
        <button 
          ...
          disabled={!hasModules}
          className="w-full py-5 bg-[#E1523D] text-white rounded-[24px] font-black uppercase tracking-[0.2em] shadow-2xl shadow-orange-900/40 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-4 disabled:opacity-30 disabled:grayscale disabled:scale-100"
        >
